...
class Lead(JarvisBase):
    ...
    outreach_eligible: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, index=True)
    review_queue: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, index=True)
    disqualification_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    signal_breakdown: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
