...
# Second-pass review budget (seconds). Bounded tightly and independently of the
# generation budget above — review is best-effort. If it can't complete in
# this window, chat() returns the unreviewed draft rather than blocking longer.
_REVIEW_TIMEOUT = 20.0

# Providers that ARE the review layer — a response drafted by one of these is
# never sent back through review (no self-review, no double Anthropic calls).
_REVIEW_PROVIDERS = {"openrouter", "anthropic", "bedrock"}

_REVIEW_SYSTEM_PROMPT = """You are the senior reviewer in a two-stage AI pipeline.
A draft response was generated by a fast production model. Your job:
1. Verify it is accurate, complete, and directly answers the original request.
2. Fix any factual errors, logical gaps, or unclear phrasing.
3. Tighten the writing — remove filler, keep it precise.
4. Preserve the original intent, format, and any required structure (JSON, lists, etc.) exactly.
Return ONLY the corrected/polished final version — no preamble, no commentary about what you changed."""
...
# ──────────────────────────────────────────────────────────────────────────────
# Two-layer architecture (Captain's directive):
#   GENERATION LAYER — NVIDIA NIM (10 rotating keys: DeepSeek, Kimi, Llama, Qwen,
#   Mistral) and Google Gemini draft every response, for every task type. This
#   is where all the volume happens, at effectively zero marginal cost.
#   REVIEW LAYER — every draft (unless it's already a review-layer response) is
#   then polished by OpenRouter Claude Opus 4.8, falling back to direct
#   Anthropic Claude Sonnet 4.6 if OpenRouter is unavailable. See chat()'s
#   `review` param and _review_and_refine() — this is applied automatically,
#   system-wide, not just here in the routing table.
...
class AIRouter:
    ...
    async def _review_and_refine(
        ...
        """
        Second-pass review: OpenRouter Opus 4.8 polishes the draft; if OpenRouter
        is unavailable, direct Anthropic Sonnet 4.6 does the same job. Best-effort
        and tightly time-boxed — any failure returns the original draft unchanged.
        Every attempt (success or failure) is recorded through the same cost/
        latency pipeline as primary generation, so review calls aren't invisible
        to AICostLedger/the AI Ops dashboard.
        """
        review_prompt = f"ORIGINAL REQUEST:\n{original_prompt}\n\nDRAFT RESPONSE TO REVIEW:\n{draft.content}"
        review_messages = [Message(role="user", content=review_prompt)]
        input_tokens = _estimate_input_tokens(review_messages, _REVIEW_SYSTEM_PROMPT)

        openrouter = self._providers.get("openrouter")
        if openrouter and openrouter.is_available():
            model_id = self._resolve_model(openrouter, "claude-opus-4-8")
            t0 = time.monotonic()
            try:
                ...
                cost = estimate_cost("openrouter", model_id, reviewed.tokens_used)
                _record_ai_metrics("openrouter", model_id, task_type, latency, cost, reviewed.tokens_used, input_tokens, reviewed.error)
                if not reviewed.error and (reviewed.content or "").strip():
                    health_monitor.record_success("openrouter", latency)
                    draft.content = reviewed.content
                    draft.reviewed_by = "openrouter/claude-opus-4.8"
                    draft.draft_provider = draft.provider
                    return draft
                health_monitor.record_failure("openrouter", latency)
            except asyncio.TimeoutError:
                health_monitor.record_failure("openrouter")
                logger.debug("Review via OpenRouter timed out after %.0fs", _REVIEW_TIMEOUT)
            except Exception as exc:
                logger.debug("Review via OpenRouter failed: %s", exc)
        ...
        if anthropic and anthropic.is_available():
            ...
            try:
                ...
                latency = int((time.monotonic() - t0) * 1000)
                cost = estimate_cost("anthropic", model_id, reviewed.tokens_used)
                _record_ai_metrics("anthropic", model_id, task_type, latency, cost, reviewed.tokens_used, input_tokens, reviewed.error)
                if not reviewed.error and (reviewed.content or "").strip():
                    health_monitor.record_success("anthropic", latency)
                    draft.content = reviewed.content
                    draft.reviewed_by = "anthropic/claude-sonnet-4-6"
                    draft.draft_provider = draft.provider
                    return draft
                health_monitor.record_failure("anthropic", latency)
            except asyncio.TimeoutError:
                health_monitor.record_failure("anthropic")
                logger.debug("Review via Anthropic timed out after %.0fs", _REVIEW_TIMEOUT)
            except Exception as exc:
                logger.debug("Review via Anthropic failed: %s", exc)

        # Both reviewers unavailable/failed — ship the unreviewed draft.
        return draft
            ...
            if provider.is_available() and model_id:
                ...
                try:
                    ...
                    if not response.error and (response.content or "").strip():
                        ...
                        if review and force_provider not in _REVIEW_PROVIDERS:
                            response = await self._review_and_refine(response, last_user_msg, max_tokens, task_type.value)
        ...
        for provider_key, model_key in route:
            ...
            if provider and provider.is_available():
                ...
                if not response.error and (response.content or "").strip():
                    ...
                    if review and provider_key not in _REVIEW_PROVIDERS:
                        response = await self._review_and_refine(response, last_user_msg, max_tokens, task_type.value)
                    return response, task_type.value
                health_monitor.record_failure(provider_key, latency)
