...
# Shared security group for all ECS tasks (voice + api services).
# Defined here (not inside the compute module) so both the database module
# and the compute module can reference it without a module dependency cycle.
...
resource "aws_lb_target_group" "voice" {
  name        = "${var.project_name}-${var.environment}-voice-tg"
  port        = 8000
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
...
# /api/* and /docs go to the api-service target group; everything else
# (Twilio webhook + WebSocket) goes to the voice-service by default.
