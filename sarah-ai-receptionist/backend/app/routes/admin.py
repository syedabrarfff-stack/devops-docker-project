...
def _sa_autobuy_skip_reason(auto_buy_twilio_number: bool, country: str) -> str | None:
    """None means proceed with auto-buy normally; a string means skip it and
    surface this warning instead.
    ...
    without one this call returns no available numbers or fails, and either
    way onboarding stalls silently at exactly the step a real clinic launch
    depends on. Production Saudi clinics keep their existing carrier number
    as the number patients dial and forward it to a Twilio number Sarah
    already holds (existing_twilio_number, checked before this runs) -- no
    purchase or port needed for that number at all, since it's never the one
    given out to patients. Surface that clearly instead of attempting a
    purchase likely to fail.
    """
    if auto_buy_twilio_number and country.upper() == "SA":
        return (
